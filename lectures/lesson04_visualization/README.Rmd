...
```{r}
ggplot() +
  geom_line(data = beerDt, aes(x = Year, y = GallonsCapita)) +
  geom_point(data = beerDt, aes(x=Year, y=GallonsCapita), color = 'blue', na.rm = TRUE)
```
...
```{r}
beerDt$myBirthYear <- beerDt$Year == 1996
ggplot(beerDt, aes(x=Year, y=GallonsCapita)) +
  geom_line() +
  geom_point(aes(color=myBirthYear))
...
ggplot(beerDt, aes(x=Year, y=GallonsCapita)) +
  geom_line() +
  geom_point(aes(fill=myBirthYear))
```
