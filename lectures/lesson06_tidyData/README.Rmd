...
```{r}
gather(untidyReview, key = 'reviewType', value ='rating', 3,4,5,6,7,8)  %>%
ggplot()+ 
  geom_bar(aes(x = rating)) +
  facet_wrap(~ reviewType)
...
gather(iphoneSales, key='Quarter', value='Sales', -year, na.rm=TRUE) %>% 
  ggplot() +
  geom_line(aes(x= Quarter, y = Sales, color = year))
```
